...
If this is the first time you open the console, you will be directed to te Developer Perspective of the console, which shows you the different namespaces you have access to. 
+
...
* Expect to see two deployments: The Globex retail application frontend (running on Node.js) and the Globex retail application itself. Note that the Globex retail application is scaled down to zero pods. As the database is missing, the application would crash if scaled up. You will scale it up once the connection with the database running on the Azure OpenShift cluster is established.
...
== Create the Red Hat Service Interconnect network

Building a Service Interconnect network between the two OpenShift clusters (more precisely between namespaces of the two OpenShift clusters) takes several steps:

* Install the Service Interconnect assets in the namespaces of both clusters.
...
* Expose services of one cluster on the other cluster. In this case, you wil expose the Globex database on the AWS cluster, so that the Globex retail app can connect to it as if it were a local service.
...
* Service Interconnect alo comes with its own console, which you can access by opening a new browser tab and navigating to %skupper_console_aws%. At the moment there is not a lot to see as we have only installed one side of the service network.
+
...
To create a link between the two clusters, you crate a token on one of the clusters, and then use the token to create the link on the other cluster.
...
+
----
bash-4.4 ~ $ cat /tmp/skupper.token 
----
+
.Output
----
...
----
+
The `globex-db` service is the proxy service created by exposing the `globex-db` deployment n the Azure cluster over the service network.
...
Congratulations! You successfully used Red Hat Service Interconnect to build a secure service network between two namespaces on two different OpenShift clusters and allow application to connect and communicate over the secure network.