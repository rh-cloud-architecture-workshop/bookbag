...
+
CAUTION: You may see channels under `CLIENTS` belonging to other students in the room. +
To prevent interferences, please ensure you select the newly created channel marked with your user name.
...
++++
+
If you see the message in Discord as above shown, then you've successfully completed this first exercise.
+
[WARNING]
====
...
. From Slack...
+
You should see the agent's message sent from Discord appear in the CamelGlobex app.

. Exchange a few more messages to simulate a conversation.

. Then, from Discord, to close the session, follow these steps, as per the illustration below:
+
--
. Right click on the channel `slack-%user_name%`
+
CAUTION: You may see channels under `CLIENTS` belonging to other students in the room. +
To prevent interferences, please ensure you select the channel with your user name.
. Click `Delete Channel`
. Confirm your action to delete the channel.
--
+
++++
<img src="./images/camel/discord-deploy-test-end-to-end.jpg" style="width:100%;border-style: none;">
++++

. In Slack, as above on the right hand side, you should see a notification informing the session has ended.

{empty} +

Well done, you've completed the full integration, both ways, between Slack and Discord.

In contrast with running in DEV mode, the `deploy.sh` command made the _Camel K_ operator to fully deploy your code in an OpenShift pod named _Discord_, which you can see running from the link:{topologyviewurl}[*_Topology_ view*, window="_blank"].
...
No IntegrationPlatform resource in globex-camel-%user_name% namespace
NAME    PHASE   KIT
discord Running globex-camel-%user_name%/kit-chcc8ts5v3ov25mqg460
----
...
[start=4]
. From Discord...
.. You should see a new channel `globex-%user_name%` created.
+
CAUTION: You may see channels under `CLIENTS` belonging to other students in the room. +
To prevent interferences, please ensure you select the channel with your user name.
...
. Back from Globex...
+
You should see the agent's message sent from Discord appear in your chat session window.
...
+
--
. Right click on the channel `globex-%user_name%`
. Click `Delete Channel`
. Confirm your action to delete the channel.
--
+
++++
<img src="./images/camel/globex-deploy-test-end-to-end.jpg" style="width:100%;border-style: none;">
++++
...
[start=3]
. Using Slack (for example) and Discord...
.. Initiate and simulate a customer/agent conversation, as done in previous exercises.

.. Then, to close the session, from Discord, right-click and delete the channel.
+
NOTE: Deleting the channel should kick-off the transcript process.
+
{empty}

. Finally, in Slack/Globex, you should see a notification informing the session has ended, plus a link to your transcript, as shown in the picture below:
+
--
. Confirm to delete the channel in Discord.
. From Slack/Globex, click on the PDF transcript link.
--
+
++++
<img src="./images/camel/transcript-test.jpg" style="width:100%;border-style: none;">
++++
...
NAME            PHASE   KIT
discord         Running globex-camel-user1/kit-chcc8ts5v3ov25mqg460
...
Please close all but this Instructions browser tab to avoid proliferation of browser tabs which can make working on other modules difficult. Click on the *Home* button below to go back to the Landing page, and choose your next module.
