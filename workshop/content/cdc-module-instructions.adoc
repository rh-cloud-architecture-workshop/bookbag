...
** *from.uri*: the source of the route, typically expressed as a URI. The scheme (*kafka*) defines which connector to use. The *{{ }}* placeholders reefer to properties defined in a properties file.
...
* The connector YAML file can be deployed as such to the OpenShift cluster using the *kamel* CLI. Under the hood the CLI will transform the YAML file into an *Integration* Custom Resource. When deployed to OpenShift, the Camel K operator processes the Integration Custom Resource which transforms the Integration into a running application.
...
+
----
kamel run -n globex-cdc-%user_name% workshop/module-cdc/customer-connector/cashback-customer-connector.yaml
----
+
.Output
----
...
----
** The Camel K operator starts to build the integration and packages it in a container image. The first time this can take quite a while during which nothing seems to happen. +
One way to check that the integration is actually being build is by checking its status with the `oc` command line tool. +
In the terminal in Dev Spaces, you can issue the following command:
+
----
oc get integration -n globex-cdc-user1
----
+
.Output
----
...
+
----
kamel run -n globex-cdc-user1 workshop/module-cdc/customer-connector/cashback-customer-connector.groovy
----
+
.Output
----
...
+
----
oc get integrationkit -n globex-cdc-user1
----
+
.Output
----
...
cashback-order-connector      Running        kit-cglu6cgm540hobmmt1r0   1
----
** The build process should be quite a lot faster than the the first one. After a while the integration proceed to the `running` phase, and becomes visible in the Topology view of the OpenShift console:
+
...
* A Kafka streams application combines and aggregates the data change event streams for orders and line_items at real time to produce a new stream of aggregated order events.
