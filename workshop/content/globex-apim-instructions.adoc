...
* Log in with your OpenShift credentials ({user_name}/{user_password}). If this is the first time you access Dev Spaces, you have to authorize Dev Spaces to access your account. In the _Authorize Access_ window click on *Allow selected permissions*. 
+
...
Keycloak is used in this module to offer Single-Sign On to Mobile users and for securing the APIs. 
...
* Launch Keycloak by clicking {sso_tenant_console}[Keycloak Console^,window="sso"] and log in using username and password ({user_name}/{user_password}).
...
In case you see an error stating *You don't have access to the requested resource* or *Network response was not OK* or any other access error, check if you are logged in as an user other than *{user_name}*. You will need to *Sign Out*, close the browser tab, and log in again.
...
* Log in as *({user_name}/{user_password})*. 
...
* This is how the *OpenID Provider Configuration* looks like:
+
...
* To get the Mobile OpenAPI draft, navigate to the browser tab with *Dev Spaces* that you have opened earlier. 
** If you don't have a browser tab open with Dev Spaces, click on {devspaces_dashboard}/dashboard/#/ide/devspaces-{user_name}/cloud-architecture-workshop[Dev Spaces IDE^, window="devspaces"]. If needed log in with your username and password ({user_name}/{user_password}).
...
* Copy the entire content from this file `(Ctrl+A and Ctrl+C)` 
+
...
| *Artifact textarea* | Click on *Browse..* button to upload the Mobile OpenAPI downloaded in the previous step, or `Drag & drop` the file into the text area.
...
* Note that the *Globex Mobile API Gateway* artifact has been uploaded and stored in the *Service Registry*.
+
...
Let us fast forward a bit in time. The backend developers team has built the Mobile Gateway server-side https://github.com/rh-cloud-architecture-workshop/globex-mobile-gateway[code^, window="code-samples"] using Quarkus. This service has been pre-deployed under the `module-apim-{user_name}` namespace on OpenShift. 


In this section you will manage and secure the Mobile Gateway API endpoints so that the Mobile App can access them securely. To create these API endpoints, secure and manage them, we will need to configure them on 3scale API management. 
...
3scale integrates with Keycloak for authenticating the API requests using the OpenID Connect specification. On signing-up, External/Mobile developers  will be provided with client credentials to access the APIs securely. This client credential is synced between 3scale and Keycloak server using a component know as *Zync*.

In order to setup OpenID Connect, you will now create a special *client id* meant for *Client Credentials Management*.

* Click to launch {sso_tenant_console}[Keycloak^, window="sso"] and log in using username and password ({user_name}/{user_password}).
* Click on *Clients* from the left-hand navigation. Then click on the *Create client* button on the right side as shown below.
...
** From the *Assign roles to client-manager* pop-up, select the *Filter by clients* dropdown, type in `manage-clients` in the *Search by role name* text box and press the *->* button to search for this manage-clients role.
+
...
* *Backends* are Internal APIs which are bundled into a 3scale Product. 
...
** If this browser tab is not open, click on {devspaces_dashboard}/dashboard/#[Dev Spaces IDE^, window="devspaces"] and log in with ({user_name}/{user_password}). 
...
* Navigate to the `globex-apim-{user_name}` namespace on the OpenShift console by clicking here {openshift_cluster_console}/topology/ns/globex-apim-{user_name}[APIM module on OpenShift^] and log in with ({user_name}/{user_password}).
...
* Under *Services* section you can see the name of the services associated with this deployment. Click on the `globex-mobile-gateway` link and you will be taken to the Services page. 
* Under *Service routing -> Hostname*, you can find the service's hostname. Make a note of this URL, as you will need this to create the Backend on 3scale.
+
...
* From the Terminal of Dev Spaces, that should be open already, run the following command: `oc whoami`. Check if you are still logged in as *{user_name}* and `oc project` to see if you are in the project `globex-apim-{user_name}`.
** If you are not in `globex-apim-{user_name}`, run `oc project globex-apim-{user_name}` to log in to the right namespace.
+
...
* In Dev Spaces, in the folder *workshop/module-apim/mobile/gateway*, open the *mobile-gateway-product.yaml* file. [Shortcut: Ctr+P > mobile-gateway-product.yaml]
...
[NOTE]
====
If you don't have this value, click on Red Hat  {sso_tenant_console}/#/globex-{user_name}/clients[Keycloak Clients List^, window="sso"]. Log in if needed with *({user_name}/{user_password})*. Click on the Client ID *client-manager*. You can copy the credentials from the *Credentials tab*.
====
+
image::images/apim/client-manager-credentials.png[]
...
* You will see the following output confirming the creation of the 3scale Product for Mobile Gateway: 
+
.Output
----
...
==== Create ActiveDoc for Mobile Gateway
...
* Create this ActiveDoc by running the following command in the Dev Spaces Terminal:
+
[source,bash,role=copy,subs="attributes"]
----
...
=== Setup Mobile users
The Mobile developers of Globex will need access to the Developer Portal to signup for the APIs exposed to them. Typically they would access the developer portal and sign up for an account,which may need to go through an approval process.

For the workshop, let us run a few commands to setup these users as defined in the file *mobile-dev-setup.yaml*.
...
* Navigate to the {3scale_tenant}[3scale admin portal^, window="3scale"] and log in using your username and password ({user_name}/{user_password}).
+
.Launch 3scale 
...
* Navigate to *Integration -> Settings* page from the Product overview page. You will notice that the Product has been set up with: 
...
image::images/apim/apim_3scale_mobile_activedoc.png[]
...
A good developer portal is a must have to assure adoption of your API. In this section we will setup the Dev Portal, so it is ready to be used by Mobile Developers.
...
* The next step is to allow a Developer to access *Multiple APIs (Services)* and sign up for *Multiple Applications*.
...
* The Globex Developer Portal is fully set up now for Mobile developers to sign up.
...
* Enter the value asterisk (*) in the **REDIRECT URL** field and click on the **Submit** button. This is to set up the right Redirect URL for OAuth using Keycloak.
...
* Execute this script in the Terminal by running the following command:
+
[source,bash,role=copy,subs="attributes"]
----
oc project globex-apim-{user_name}
...
----
* The Mobile App Deployment is patched with the necessary variables. You can view this by navigating to {openshift_cluster_console}/k8s/ns/globex-apim-{user_name}/deployments/globex-mobile/environment[globex-mobile deployment, window="console"].
+
.globex-mobile deployment on OpenShift
image::images/apim/apim_globex_mobile_deployment.png[]

==== Update Keycloak's Web Origins to match Mobile App
There is one last step that you need to do before trying out the Mobile App. You need to update the *Web Origins*.

* Navigate to click on {sso_tenant_console}/#/globex-{user_name}/clients[Keycloak Clients List^,window="sso"]. Log in if needed with *({user_name}/{user_password})*.
+
...
The Mobile Application can be accessed via this QR Code as well as via browser
...
=== Under the Hood
As part of this vection you tried out the Mobile App. 
...
** Log in if needed with *({user_name}/{user_password})*. 

[WARNING]
====

In case you see an error stating *You don't have access to the requested resource* or *Network response was not OK* or any other access error, check if you are logged in as a user other than *{user_name}*. You will need to *Sign Out*, close the browser tab, and log in again.
...
=== Create ActiveDoc for Partner Gateway
...
* Create this ActiveDoc by running the following command in the Terminal:
+
[source,bash,role=copy,subs="attributes"]
----
...
=== Sign Up for an Application as a Partner
...
+
.Updated secret.yaml
image::images/apim/partner_secret_yaml_updated.png[]
...
In the previous section, you signed up for access as a Partner Developer and gained credentials to access the Globex Partner Gateway API. You also set up the necessary configurations for the Partner Web Application.
...
* Log in using username/password as *(partner/openshift)*. 
...
=== Under the Hood
As part of this section you set up and tested the Partner Web App. 
