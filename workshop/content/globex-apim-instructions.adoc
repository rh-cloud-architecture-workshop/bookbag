...
3scale integrates with Keycloak for authenticating the API requests using the OpenID Connect specification. On signing-up, External/Mobile developers  will be provided with client credentials to access to the APIs securely. This client credentials is synced between 3scale and Keycloak server using a component know an *Zync*
...
* The next step is to configure this `client-manager` so that 3scale can synchronize with Keycloak, so that Keycloak can manage other clients (create, amend and delete) on behalf of 3scale API Management
...
** Open the {sso_tenant_console}/#/realms/globex-{user_name}/clients[SSO Clients List^, window="sso"] from Keycloak.
...
* Click on *Sign Out* and close the browser
* Reopen the {sso_tenant_console}/#/realms/globex-{user_name}/clients[Keycloak Clients List^, window="sso"]. 
* Login as *({user_name}/{user_password})*. 
