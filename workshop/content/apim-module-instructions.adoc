...
We will be covering a lot of ground in this module. There are two distinct sections in this module

* Setting up Mobile App: Red Hat SSO' OpenID Connects provides user SSO logins and also secure access to backend  systems. 
* Setting up Partner Portal Access: Users are managed by partners themselves, and Red Hat SSO' OpenID Connects only secures access to backend  systems. 
...
** Turn on Content validations
** Explore how OpenAPI can be used by the ecosystem
...
** Don't close this browser tab. You will use the Mobile OpenAPI YAML content from the API Designer in the next step.
+
...
* When an external developer (in this case Mobile developer) signs up for an API, they will be provided with a client-id and client-secret which will need to be used to access the APIs securely. 
...
In the next step you will setup Red Hat SSO so that 3scale will be able to synchronise the client credentials with SSO.
...
----
* From the Terminal of Dev Spaces that should be open already
...
* In Dev Spaces in the same folder as the previous steps `workshop -> module-apim -> mobile -> gateway`, open the file `mobile-gateway-product.yaml`. This file creates a 3scale product, and also attached to it the Backend we created in the previous step.
...
[NOTE]
====
If you don't have this value, click on {sso_tenant_console}/#/realms/globex-{user_name}/clients[SSO Clients List]. Login if needed with *({user_name}/{user_password})*. Click on the Client ID *client-manager*. You can copy the credentials from the *Credentials tab*
====
+
image::images/apim/client-manager-credentials.png[]
...
* The ActiveDoc is visible from the 3scale portal as well under Products. <tbc - do we need this? refine>
+
...
* After updation, this page should be seen as per the screenshot below. 
+
...
** Substitute *<client-credentials>* with these Credentials.
